...
const SALES_AGENT_PROFILES: Record<string, SalesAgentProfile> = {
  'sales-agent': {
    id: 'sales-agent',
    label: 'Sales Agent',
    description: 'Spezialisiert auf Verkauf, Lead-Qualifizierung, Angebotserstellung und Abschlüsse.',
    goals: [
      'Kundenbedürfnisse verstehen und passende Produkte empfehlen',
  ...
  'product-expert': {
    ...
    goals: [
      ...
      'Use Cases und Best Practices erklären',
      'Technische Details verständlich kommunizieren',
    ],
    allowedActions: [
      'product_details',
      'feature_comparison',
...
const PRODUCTS: Product[] = [
  {
    ...
    description: 'DSGVO-konformer WhatsApp Business Bot Builder - Erstelle AI-gestützte WhatsApp Bots ohne Code',
    price: 29,
    currency: 'EUR',
    features: [
      'Visueller Bot-Builder (Drag & Drop)',
      'AI-gestützte Konversationen',
...
`).join('\n');
    ...
    const prompt = `
Du bist ${selectedAgent.label} für whatsapp.owona.de.
...
**Erlaubte Aktionen:**
${selectedAgent.allowedActions.map((a) => `- ${a}`).join('\n')}
...
- Empfehle passende Produkte basierend auf Kundenbedürfnissen
...
Antworte als ${selectedAgent.label} und helfe dem Kunden weiter.
`;
    ...
    if (deepseekApiKey) {
      ...
      try {
        ...
        const prompt = `Du bist ein professioneller Sales/Marketing-Assistent für whatsapp.owona.de. Antworte immer auf Deutsch, freundlich und verkaufsorientiert.
...
- Wenn der Assistent fragt "Möchtest du mehr Details zu einem bestimmten Feature?" und der Kunde "ja" sagt, dann frage nach welchem Feature
...
Antworte als ${selectedAgent.label} und helfe dem Kunden weiter. Antworte NUR mit der Antwort, keine zusätzlichen Erklärungen.`;

        console.log('[Sales Chat Edge Function] Sending to DeepSeek:', {
        ...
        console.error('[Sales Chat Edge Function] DeepSeek API Error:', {
      ...
      // Fallback ohne LLM
      console.warn('[Sales Chat Edge Function] DEEPSEEK_API_KEY nicht gesetzt - verwende Fallback-Antworten');
      response = {
        response: generateFallbackResponse(message, selectedAgent, conversationHistory),
        agent: selectedAgent.id,
      };
...
function generateFallbackResponse(message: string, agent: SalesAgentProfile, conversationHistory: Array<{ role: 'user' | 'assistant'; content: string }> = []): string {
  ...
  if ((messageLower === 'ja' || messageLower === 'yes' || messageLower === 'ok' || messageLower === 'gerne' || messageLower === 'klar') && lastAssistantMessage) {
    if (lastAssistantMessage.includes('Link zur Registrierung') || lastAssistantMessage.includes('Registrierung schicken') || lastAssistantMessage.includes('Link schicken')) {
      return `Perfekt! 🎉 Hier ist der Link zur Registrierung:\n\n👉 https://whatsapp.owona.de/de/auth/signup\n\nNach der Registrierung kannst du sofort mit der 14-tägigen kostenlosen Testversion starten!`;
    }
    if (lastAssistantMessage.includes('Testversion starten') || lastAssistantMessage.includes('Testversion')) {
      return `Super! 🚀 Du kannst dich hier registrieren:\n\n👉 https://whatsapp.owona.de/de/auth/signup\n\nNach der Registrierung startet automatisch deine 14-tägige kostenlose Testversion!`;
    }
